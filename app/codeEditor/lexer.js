...
function generatePreProcessor(input) {
    ...
    if (singleLinePP) {
        ...
        throw ("Unable to generate PRE_PROCESSOR from beginning of input.");
    }
}
...
function generateWhiteSpace(input) {
    var whiteSpaceToKind = new Map([
        ["\t", "TAB"],
        ["\n", "NEWLINE"],
        ["\v", "VERTICAL_TAB"],
        ["\f", "FORM_FEED"]
...
function generateSingleToken(input) {
    try {
        var tokenMatches = tryEach(
            [generatePreProcessor, generateComment, generateKeywords, generateIdentifier, generateFloatCnst, generateIntCnst, generateStringLiteral, generateOperator, generateSpecialChar,
                generateWhiteSpace
            ], input);
...
function generateTokens(input, accumulator = []) {
    if (input === "") {
        return accumulator;
    }
    else {
        ...
        return generateTokens(newInput, accumulator);
    }
}
